...
import ru.craftlogic.api.util.PooledLongQueue;
...
public class LightingEngineFast implements LightingEngine {
    ...
    public LightingEngineFast(final World world) {
        this.world = world;
    ...
    private void acquireLock() {
        if (!this.lock.tryLock()) {
            ...
            if (current != this.ownedThread) {
                throw new RuntimeException(String.format("World is owned by '%s' (ID: %s), but was accessed from thread '%s' (ID: %s)",
    ...
    private int getCursorCachedLight(final EnumSkyBlock lightType) {
        return ((ChunkLighting) this.curChunk).getCachedLightFor(lightType, this.curPos);
    }
